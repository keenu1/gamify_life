    ...
    <>
      <aside className={`hidden md:block bg-white h-screen shadow-lg rounded-r-xl absolute top-0  transition-all duration-500 ease-in-out ${showMenu ? "w-1/5  " : "w-16"}`}>
        <nav className="flex flex-col justify-between  items-center h-full">
            ...
            <div className="flex justify-between  items-center w-full">
              <div className={`flex items-center   w-full font-semibold`}>
                <div className="flex items-center gap-2 ">
                  ...
                  />
                  <span className={`ms-2 transition-all md:text-transparent md:text-xs lg:text-lg  flex-none  lg:text-black ${showMenu ? ' delay-300 duration-200 ease-in-out opacity-100 translate-x-0 ' : 'opacity-0 duration-0 translate-x-[-10px]'}`}>
                    Gamify Life
                  </span>
                </div>

              </div>
              <div className={`bg-gray-100 rounded-lg z-10  px-1 hover:cursor-pointer hover:bg-gray-200 ${showMenu ? "" : "hidden"}`}>
          ...
          <div className="p-3 w-full relative ">
            ...
            <div
              className={`absolute bottom-0 mb-3 -me-2   z-10 mt-2 w-full -right-full origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none  ${dropdownProfile
      ...
      <aside className={`border block md:hidden bg-white w-screen shadow-lg rounded-t-xl absolute bottom-0  transition-all duration-500 ease-in-out ${showMenu ? "w-1/5  " : "w-16"}`}>
        <nav className="flex relative flex-row justify-between  items-center h-full">
          ...
          <div className={`flex flex-row gap-2 ${showMenu ? "" : ""}`}>
            <Link
              to="/home"
              className={`${isActive("/home") ? "bg-gray-100 text-black font-medium " : "text-gray-500 hover:font-medium hover:text-black "
                } relative flex items-center rounded-lg px-3 py-2`}
            >
              ...
              <span className={`ms-6 flex-none absolute   transition-all  ${showMenu ? "opacity-100 transform translate-x-0 duration-200 delay-300" : "opacity-0 translate-x-[-20px] duration-0 "}`}>
                {/* Home */}
              </span>
            </Link>
            <Link
            ...
            >
              <i className='bx bxs-dashboard' ></i>
              <span className={`ms-6 flex-none absolute   transition-all  ${showMenu ? "opacity-100 transform translate-x-0 duration-200 delay-500" : "opacity-0 translate-x-[-20px] duration-0 "}`}>
                {/* Dashboard */}
              </span>
            </Link>
            <Link
          ...
          <div
            className={`absolute right-0 bottom-14 mb-3 ms-52  z-10 mt-2 w-56 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none  ${dropdownProfile
              ? "transform opacity-100 scale-100 transition ease-out duration-100 "
              : "transform opacity-0 scale-95 transition ease-in duration-75  pointer-events-none"
              }`}
            role="menu"
